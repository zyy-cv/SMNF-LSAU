    ...
    def collate_fn(self, datas):
        ...
        if self.inp_size is None:
            ...
            for idx in range(len(self.scale_max)):
                ...
                scale = torch.tensor(self.scale_it[0])
        ...
        cell[0] *= 2. / img_h
        cell[1] *= 2. / img_w
        cell = cell.unsqueeze(0).repeat(self.inp_size**2, 1)
        cell = cell.unsqueeze(0).repeat(inp.shape[0], 1,1)
        ...
        return {'inp': inp, 'gt': hr_rgb, 'coords': coords[0], 'cell': cell, 'sample_coord': sample_coord,'scale': scale}
...
@register('test-sr-implicit-downsampled')
class SRImplicitDownsampledCascaded(Dataset):
    def __init__(
        self,
        dataset,
    ...
    def collate_fn(self, datas):
        ...
        if self.inp_size is None:
            ...
            for idx in range(len(self.scale_it)):
                ...
                #coord = make_coord((img_h, img_w), flatten=False) \
                #                    .unsqueeze(0).repeat(self.batch_size, 1, 1, 1)
                coord = make_coord((img_h, img_w)).unsqueeze(0)
        ...
        cell[0] *= 2. / img_h
        cell[1] *= 2. / img_w
        cell = cell.unsqueeze(0).repeat(img_h * img_w, 1)
        cell = cell.unsqueeze(0).repeat(inp.shape[0], 1, 1)
...
@register('test-sr-implicit-paired')
class SRImplicitPairedCascaded(Dataset):
    def __init__(
        self,
        dataset,
    ...
    def collate_fn(self, datas):
        ...
        s = torch.tensor(s)
        return {'inp': inp, 'gt': hr_rgb, 'coords': coords, 'cell': cell, 'sample_coord': sample_coord,'scale':s}
